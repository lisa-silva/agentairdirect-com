(function () {
  ...
  function render() {
    ...
    root.querySelector("#roi-return-label").textContent = "Scenario Return on Investment";
    ...
    root.querySelector("#roi-assumptions").textContent = customerNumber.format(result.customers) + " monthly customers × " + currency.format(result.ticket) + " average ticket. User-assumed increase in AI-referred customers: +" + number.format(result.visibilityLift) + "%. User-assumed increase in customer conversion: +" + number.format(result.conversionLift) + "%." + (result.investment > 0 ? " Planned investment: " + currency.format(result.investment) + "." : " Add your own planned investment to explore the scenario.");
  }
